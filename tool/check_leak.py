...
def run_one_check(max_call_depth, ops, contract_address,
    ...
    execute_one_block(ops, stack, 0, trace, storage, mmemory, data, configurations, [
                      'CALL', 'SUICIDE'], ether_leak, 0, 0, debug, read_from_blockchain)


def check_one_contract_on_ether_leak(
        contract_bytecode,
        contract_address,
        debug=False,
    ...
    if not code_has_instruction(ops, ['CALL', 'SUICIDE']):
        # if debug:
        print(
            '\033[92m[+] The code does not have CALL/SUICIDE, hence it is not prodigal\033[0m')
        return False
    if debug:
    ...
    if MyGlobals.stop_search:
        ...
        print(
            '\n\n\033[91m[-] Leak vulnerability found! \033[0m\n\n    The following %d transaction(s) will trigger the contract to leak:' %
            MyGlobals.no_function_calls)
        ...
        if len(fhashes) > 0:
            ...
            for n in range(MyGlobals.no_function_calls):
                if MyGlobals.function_calls[n + 1]['input'][:8] in fhashes:
                    print(
                        '    -' + fhashes[MyGlobals.function_calls[n + 1]['input'][:8]])
            print()

        if confirm_exploit:

            print(
                '\033[1m[ ] Confirming leak vulnerability on private chain ... \033[0m',
                end='')
            ...
            if difference_in_wei > 0:
                ...
                print(
                    '\033[94m    Cannot confirm the leak vulnerability \033[0m')

        else:
            print(
                '\033[94m[-] Cannot confirm the bug because the contract is not deployed on the blockchain.\033[0m')
