...
def print_stack(stack):
    ...
    for s in stack[::-1]:
        if 'z3' in s:
            if is_bv_value(simplify(s['z3'])):
                print('%10s : %4x  : %x' %
                      (s['type'], s['step'], simplify(s['z3']).as_long()))
            else:
                print('%10s : %4x  : %s' %
                      (s['type'], s['step'], simplify(s['z3'])))
        else:
...
def print_storage(storage):
    ...
    for fl in storage:
        for s in storage[fl]:
            ...
            if is_bv_value(simplify(s['z3'])):
                print('%x' % (simplify(s['z3']).as_long()))
            else:
                print('%s' % (simplify(s['z3'])))
...
def print_memory(mmemory):
    ...
    for m in mmemory:
        ...
        if fl['type'] == 'undefined':
            print('undefined')
        elif is_bv_value(simplify(fl['z3'])):
            print('%x' % (simplify(fl['z3']).as_long()))
        else:
            print('%s' % (simplify(fl['z3'])))
...
def get_function_calls(calldepth, debug):
    ...
    if MyGlobals.s.check() == sat:
        ...
        for cd in range(1, calldepth + 1):
            ...
            next_free = 0
            for f in range(100):
                if ('input' + str(cd) + '[' + str(4 + 32 * f) + ']') in sol or (
                        'input' + str(cd) + '[' + str(4 + 32 * f) + ']d') in sol:
                    next_free = 32 * f + 32
            ...
            if len(tmp_one[function_hash][:-56]) > 0:
                function_inputs[cd] = '%08x' % int(
                    tmp_one[function_hash][:-56], 16)
            else:
            ...
            for offset in range(100):
                ...
                if addr in tmp_one:
                    function_inputs[cd] = function_inputs[cd] + \
                        '%064x' % int(tmp_one[addr], 16)
        ...
        for n in range(10):
            if n in function_inputs:
                ...
                MyGlobals.function_calls[n] = {
                    'input': function_inputs[n], 'value': call_value}
        ...
        if debug:
            for n in range(calldepth):
                print('- %d - %10x -  ' %
                      (n + 1, MyGlobals.function_calls[n + 1]['value']), end='')
                for j in range(len(MyGlobals.function_calls[n + 1]['input'])):
                    if (j - 8) % 64 == 0:
                        print(' ', end='')
                    print('%s' %
                          MyGlobals.function_calls[n + 1]['input'][j], end='')
                print('')
