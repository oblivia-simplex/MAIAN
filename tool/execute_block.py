...
def execute_one_block(
    ...
    if MyGlobals.visited_nodes > MyGlobals.MAX_VISITED_NODES:
        return
    ...
    while (first or newpos != pos) and not MyGlobals.stop_search:
        ...
        if pos == 0:
            calldepth += 1
            jumpdepth = 0
        if ops[pos]['o'] == 'JUMPDEST':
            jumpdepth += 1
        if(jumpdepth > MyGlobals.MAX_JUMP_DEPTH):
            if debug:
                print('\033[95m[-] Reach MAX_JUMP_DEPTH\033[0m')
            return
        if(calldepth > MyGlobals.MAX_CALL_DEPTH):
            if debug:
                print('\033[95m[-] Reach MAX_CALL_DEPTH\033[0m')
            return
        ...
        if pos == 0 or ops[pos]['o'] == 'JUMPDEST' or (
                pos > 0 and ops[pos - 1]['o'] == 'JUMPI'):
            if seen_configuration(configurations, ops, pos,
                                  stack, mmemory, storage):
                if debug:
                    print('\033[95m[-] Seen configuration\033[0m')
                return
        ...
        if ops[pos]['o'] in search_op:
            ...
            if debug:
                print('\033[96m[+] Reached %s at %x \033[0m' %
                      (ops[pos]['o'], ops[pos]['id']))
                print_stack(stack)

            new_search_condition_found, stop_expanding_the_search_tree = search_function(
                ops[pos]['o'], stack, trace, debug)
            ...
            if MyGlobals.stop_search or stop_expanding_the_search_tree:
                return

        # Execute the next operation
        newpos, halt = execute(ops, stack, pos, storage, mmemory,
                               data, trace, calldepth, debug, read_from_blockchain)

        # If halt is True, then the execution should stop
        if halt:

            if debug:
                print('\033[94m[+] Halted on %s on line %x \033[0m' %
                      (ops[pos]['o'], ops[pos]['id']))
        ...
        if pos == newpos:
            ...
            if si['o'] == 'JUMPI':
                ...
                if len(stack) < 2:
                    if debug:
                        print(
                            '\033[95m[-] In JUMPI (line %x) the stack is too small to execute JUMPI\033[0m' %
                            pos)
                    return False
                ...
                # In the fast search mode, the jumpi pos + 1 must be in the
                # list of good jump positions
                ...
                if not is_fixed(addr):
                    if debug:
                        print(
                            '\033[95m[-] In JUMPI the jump address cannot be determined \033[0m' %
                            jump_dest)
                    return False
                ...
                # In the fast search mode, the jumpi new_position must be in
                # the list of good jump positions
                        ...
                        if MyGlobals.s.check() == sat:
                            ...
                            if debug:
                                if ops[pos]['id'] - MyGlobals.last_eq_step < 5:
                                    print(
                                        '\t' * 8 + '-' * 18 + '\033[96m %2d Executing function %x \033[0m' %
                                        (calldepth, MyGlobals.last_eq_func))
            ...
            # It can be CALLDATALOAD
                ...
                if match:
                    ...
                    # add 'd' at the end of the name of the symbolic variable
                    # (used later to distinguish them)
                    if index >= 0 and (
                            'data-' + str(calldepth) + '-' + str(index)) in data:
                        data[('data-' + str(calldepth) + '-' + str(index))
                             ] = BitVec(sm + 'd', 256)
                        MyGlobals.s.push()
                        MyGlobals.s.add(
                            data[('data-' + str(calldepth) + '-' + str(index))] == random_address)
                        total_added_to_solver = 1

                    # replace the variable with concrete value in stack and
                    # memory
                    for st in stack:
                        if 'z3' in st:
                            st['z3'] = simplify(
                    ...
                    for one_branch_size in branch_array_size:
                        ...
                        data2['data-' + str(calldepth) + '-' + str(addr)
                              ] = BitVecVal(one_branch_size, 256)
                        ...
                        stack2.append({'type': 'constant', 'step': ops[pos]['id'], 'z3': BitVecVal(
                            one_branch_size, 256)})

                        MyGlobals.s.push()
                        MyGlobals.s.add(BitVec('input' +
                                               str(calldepth) +
                                               ('[%x' %
                                                addr.as_long()) +
                                               ']', 256) == one_branch_size)
            ...
            # It can be CALLDATASIZE
                ...
                if -1 not in data2:
                    data2['inputlength-' +
                          str(calldepth)] = BitVec('inputlength-' +
                                                   str(calldepth), 256)
                stack2.append({'type': 'constant',
                               'step': ops[pos]['id'],
                               'z3': data2['inputlength-' + str(calldepth)]})
                execute_one_block(
                    ops,
                    stack2,
                    pos + 1,
            ...
            # If nothing from above then stop
                ...
                print('\033[95m[-] Unknown %s on line %x \033[0m' %
                      (si['o'], ops[pos]['id']))
                return
