...
def start_private_chain(chain, etherbase, debug=False):
    ...
    if chain != 'remptychain':
        ...
        # Copy the accounts
        pr = subprocess.Popen(['cp',
                               '-r',
                               './blockchains/remptychain/keystore',
                               './blockchains/' + chain + '/'])
        pr.wait()
    ...
    print('\033[1m[ ] Connecting to PRIVATE blockchain %s  \033[0m' %
          chain, end='')
    if debug:
        pro = subprocess.Popen(['geth',
                                '--rpc',
                                '--rpccorsdomain',
                                '"*"',
                                '--rpcapi="db,eth,net,web3,personal,web3"',
                                '--rpcport',
                                MyGlobals.port_number,
                                '--datadir',
                                'blockchains/' + chain,
                                '--networkid',
                                '123',
...
def kill_active_blockchain():
    ...
    devnull = open(os.devnull, 'w')
    p = subprocess.Popen(['fuser', MyGlobals.port_number + '/tcp'],
    ...
    p = subprocess.Popen(['lsof', '-t', '-i', 'tcp:' + MyGlobals.port_number],
                         stdout=subprocess.PIPE, stderr=devnull)
    out, err = p.communicate()
    for line in out.splitlines():
        pid = int(line.split(None, 1)[0])
        p2 = subprocess.Popen(['ps', '-p', str(pid)],
                              stdout=subprocess.PIPE, stderr=devnull)
...
def execute_transactions(txs):
    ...
    for tx in txs:
        ...
        try:
            ...
            weiused += MyGlobals.web3.eth.getTransactionReceipt(
                hash)['gasUsed'] * MyGlobals.web3.eth.getTransaction(hash)['gasPrice']
