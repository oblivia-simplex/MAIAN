    ...
    for i in range(addr + 1, addr + 32):
        if i in mmemory:
            ...
            if not is_undefined(mmemory[i]):
                ...
                new_value = (old_value & (2**(8 * obytes) - 1)
                             ) ^ (value['z3'] << (8 * obytes))
...
def unary(o1, step, op='NONE'):
    ...
    if op == 'NOT':
        ...
        z3 = If(z1 == 0, BitVecVal(1, 256), BitVecVal(0, 256))
    else:
...
def binary(o1, o2, step, op='NONE'):
    ...
    # In some cases the result can be determined with the knowledge of only
    # one operand
    if is_fixed(o1):
...
def ternary(o1, o2, o3, step, op='NONE'):
    ...
    if o3['type'] == 'constant' and is_bv_value(
            simplify(o3['z3'])) and 0 == simplify(o3['z3']).as_long():
        return {'type': 'constant', 'step': step, 'z3': BitVecVal(0, 256)}
    ...
    if op == 'ADDMOD':
        return {'type': 'constant', 'step': step, 'z3': (z1 + z2) % z3}
    elif op == 'MULMOD':
        return {'type': 'constant', 'step': step, 'z3': (z1 * z2) % z3}
    else:
...
def is_good_jump(ops, pos, debug):
    ...
    if debug:
        print('\033[95m[-] Bad jump :%x\033[0m' % ops[pos]['id'])
    return False


def execute(code, stack, pos, storage, mmemory, data,
            trace, calldepth, debug, read_from_blockchain):
    op = code[pos]['o']
    halt = False
    executed = True
    step = code[pos]['id']
    ...
    if allops[op][1] > len(stack):
        if debug:
            print(
                'Not enough entries in the stack to execute the operation %8s  at step %x: required %d, provided %d' %
                (op, code[pos]['id'], allops[op][1], len(stack)))
        return pos, True
    ...
    # all ternary
        ...
        if exact_address >= 0 and exact_offset >= 0:
            if (exact_offset %
                    32) == 0:     # for now, can deal only with offsets divisible by 32
                ...
                if all_good:
                    ...
                    digest = k.hexdigest()
                    res = {'type': 'constant', 'step': step,
                           'z3': BitVecVal(int(digest, 16), 256)}

        if MyGlobals.symbolic_sha and is_undefined(res):
            res = {'type': 'constant', 'step': step, 'z3': BitVec(
                'sha-' + str(step) + '-' + str(calldepth), 256)}

        stack.append(res)

    elif op.find('PUSH') >= 0:
        stack.append({'type': 'constant', 'step': step,
                      'z3': BitVecVal(int(code[pos]['input'], 16), 256)})
    elif op.find('DUP') >= 0:
    ...
    # otherwise, below, they will get fixed value (BitVecVal) as specified
        ...
        stack.append({'type': 'constant', 'step': step,
                      'z3': BitVec(op + '-' + str(calldepth), 256)})

    elif op == 'NUMBER':
        stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(
            int(get_params('block_number', ''), 16), 256)})
    elif op == 'GASLIMIT':
        stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(
            int(get_params('gas_limit', ''), 16), 256)})
    elif op == 'TIMESTAMP':
        stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(
            int(get_params('time_stamp', ''), 16), 256)})
    elif op == 'CALLVALUE':
        stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(
            int(get_params('call_value', ''), 16), 256)})
    elif op == 'ADDRESS':
        stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(
            int(get_params('contract_address', ''), 16), 256)})
    elif op == 'ORIGIN':
        stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(
            int(get_params('contract_address', ''), 16), 256)})
    elif op == 'GASPRICE':
        stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(
            int(get_params('gas_price', ''), 16), 256)})
    elif op == 'COINBASE':
        stack.append({'type': 'constant', 'step': step,
                      'z3': BitVecVal(0, 256)})
    elif op == 'DIFFICULTY':
        ...
        stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(
            int(get_params('my_address', ''), 16), 256)})
    elif op == 'GAS':
        stack.append({'type': 'constant', 'step': step,
                      'z3': BitVecVal(int(get_params('gas', ''), 16), 256)})
    elif op == 'MSIZE':
        stack.append({'type': 'constant', 'step': step,
                      'z3': BitVecVal(len(mmemory), 256)})
    elif op == 'BLOCKHASH':
        # does not use the argument which specifies the blocknumber
        stack.append({'type': 'constant', 'step': step,
                      'z3': BitVecVal(0x123, 256)})
    elif op == 'BALANCE':
        # always assume that it is the balance of the current contract
        stack.append({'type': 'constant', 'step': step, 'z3': BitVecVal(
            int(get_params('contract_balance', ''), 10), 256)})
    elif op == 'POP':
        pass
    elif op.find('LOG') >= 0:
        pass
        ...
        if is_fixed(addr):
            ...
            stack.append({'type': 'constant', 'step': step,
                          'z3': data['data-' + str(calldepth) + '-' + str(addr)]})

        elif is_undefined(addr):

            if debug:
                print(
                    '\033[95m[-] In CALLDATALOAD the input address cannot be determined at step %x: \033[0m' %
                    code[pos]['id'])
                print(addr)
            return pos, True
        ...
        # assume the result of call can be any (True or False)
        stack.append({'type': 'constant', 'step': step,
                      'z3': BitVec('call_at_step_' + str(step), 256) & 0x1})
        ...
        if not is_fixed(memaddr) or not is_fixed(
                datapos) or not is_fixed(length):
            if debug:
                print(
                    '\033[95m[-] In CALLDATACOPY the memory address or datapos or length cannot be determined \033[0m')
                print(memaddr)
                print(datapos)
                print(length)
            return pos, True
        ...
        if length % 32 != 0:
            if debug:
                print(
                    '\033[95m[-] In CALLDATACOPY the length of array (%d) is not multiple of 32 \033[0m' %
                    length)
            return pos, True
            ...
            data[datapos +
                 ...
                 i] = BitVec('input' +
                             ...
                             str(datapos +
                                 ...
                                 i) +
                             ']', 256)
            store_in_memory(mmemory,
                            memaddr + 32 * i,
                            {'type': 'constant',
                             'step': step,
                             'z3': data[datapos + 32 * i]})
        ...
        if is_undefined(addr):
            if debug:
                print(
                    '\033[95m[-] The MLOAD address on %x  cannot be determined\033[0m' %
                    code[pos]['id'])
            return pos, True
        ...
        if is_undefined(addr) or not is_bv_value(simplify(addr['z3'])):
            if debug:
                print(
                    '\033[95m[-] The MSTORE the write address on %x  cannot be determined\033[0m' %
                    code[pos]['id'])
            return pos, True
        ...
        if not is_fixed(addr):
            if debug:
                print(
                    '\033[95m[-] The MSTORE8 the write address on %x  cannot be determined\033[0m' %
                    code[pos]['id'])
            return pos, True
        if not is_fixed(value):
            if debug:
                print(
                    '\033[95m[-] The MSTORE8 value is undefined \033[0m' %
                    code[pos]['id'])
            return pos, True
        ...
        if is_undefined(addr):
            if debug:
                print(
                    '\033[95m[-] The SLOAD address on %x  cannot be determined\033[0m' %
                    code[pos]['id'])
            return pos, True
        ...
        if is_bv_value(addr):
            ...
            if exact_address in storage:
                ...
                if total_values == 0:
                    print('In SLOAD the list at address %x has no elements ' %
                          exact_address)
                    exit(0)
                    return pos, True
                else:
                ...
                if MyGlobals.web3 is not None and read_from_blockchain:
                    value = MyGlobals.web3.eth.getStorageAt(
                        get_params('contract_address', ''), exact_address)
                else:
                    value = '0'

                t = {'type': 'constant', 'step': step,
                     'z3': BitVecVal(int(value, 16), 256)}
            ...
            if MyGlobals.symbolic_load:
                stack.append({'type': 'constant', 'step': step, 'z3': BitVec(
                    'sload-' + str(step) + '-' + str(calldepth), 256)})
            else:
                if debug:
                    print(
                        '\033[95m[-] The SLOAD address on %x  cannot be determined\033[0m' %
        ...
        if is_undefined(addr):
            if debug:
                print(
                    '\033[95m[-] The SSTORE address on %x  cannot be determined\033[0m' %
                    code[pos]['id'])
            return pos, True
        ...
        if is_bv_value(simplify(addr['z3'])):
            ...
            if MyGlobals.symbolic_load:
                ...
                if debug:
                    print(
                        '\033[95m[-] In SSTORE the write address cannot be determined at step %x: \033[0m' %
                        code[pos]['id'])
                    print(addr)
                return pos, True
        ...
        if not is_fixed(addr):
            if debug:
                print(
                    '\033[95m[-] In JUMP the address cannot be determined \033[0m')
            return pos, True
        ...
        if(jump_dest <= 0):
            if debug:
                print(
                    '\033[95m[-] The JUMP destination is not a valid address : %x\033[0m' %
                    jump_dest)
            return pos, True
        ...
        if is_undefined(word) or is_undefined(byte_no):
            ...
            res = {'type': 'constant', 'step': step, 'z3': (
                word['z3'] >> (8 * (31 - byte_no['z3']))) & 0xff}
    ...
    if executed and final_stack_size != len(stack):
        print(
            'Incorrect final stack size after executing %s at step %x' %
            (op, step))
        print(len(stack))
