...
def initialize_params(read_from_blockchain, c_address):
    ...
    if read_from_blockchain:
        MyGlobals.st['contract_balance'] = str(
            MyGlobals.web3.eth.getBalance(c_address) + 1).zfill(64)
    else:
...
def add_configuration(step, configurations, nc):
    ...
    if step in configurations:
        configurations[step].append(nc)
    else:
        configurations[step] = [nc]
...
def configuration_exist(step, configurations, nc):
    ...
    for os in configurations[step]:
        ...
        # Compare stack
        if os['stack'] != nc['stack']:
            continue

        # Compare mmemory
        if os['mmemory'] != nc['mmemory']:
            continue

        # Compare storage
        if(os['storage'] != nc['storage']):
            continue
...
class MyGlobals(object):
    ...
    # sum of all paths in search of one contract
    ...
    chk = Web3.toChecksumAddress
    etherbase_account    = chk('69190bde29255c02363477462f17e816a9533d3a')
