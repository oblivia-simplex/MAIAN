...
import json
import rlp
from eth_utils.address import decode_hex, encode_hex
...
def derive_solc_out_path(out_dir, source_path, contract_name=None):
    """
    Take the path of the source file, and return a pair containing the
    paths to the bin and abi files generated by solc.
    """
    f = "_".join(source_path.replace("/","_").split("."))
    if contract_name is not None:
        f += '_' + contract_name
    base = os.path.join(out_dir, f)
    return (base+'.bin', base+'.abi')
...
def compile_contract(filename):
    ...
    out_dir = 'out'
    p = Popen(['solc', '--bin', '--abi', '-o', out_dir, source_file,
...
def get_function_hashes(abi_path):
...
def deploy_contract(etherbase,
    ...
    if abi_path is None:
        with open(bin_path, 'r') as myfile:
            byt = myfile.read()
        filename_write_address = os.path.basename(bin_path) + '.address'
