...
def compile_contract(filename):
    ...
    print(
        '\033[1m[ ] Compiling Solidity contract from the file %s ... \033[0m' %
        filename,
        end='')

    source_file = filename
    if (not os.path.isfile(source_file)):
        print('\033[91m[-] Contract file %s does NOT exist\033[0m' %
              source_file)
        return
    ...
    p = Popen(['solc', '--bin', '--abi', '-o', 'out', source_file,
               '--overwrite'], stdout=PIPE, stdin=PIPE, stderr=STDOUT)
    solo = ''
...
def deploy_contract(filename, etherbase, rawcode=False):
    ...
    if rawcode:
        ...
        filename_write_address = os.path.basename(filename) + '.address'
        with open('./out/' + filename_abi, 'r') as myfile:
            abi = myfile.read()
        with open('./out/' + filename_bin, 'r') as myfile:
            byt = myfile.read()
        if(len(abi) == 0 or len(byt) == 0):
            print(
                '\033[91m[-] Some of the files is missing or empty: |%s|=%d  |%s|=%d' %
                (filename_abi, len(abi), filename_bin, len(byt)))
    ...
    print('\033[1m[ ] Contract code length on the blockchain : \033[0m \033[92m  %d  : %s... \033[0m' % (
        len(fullcode), fullcode[:30]))
    with open('./out/' + filename_write_address, 'w') as f:
        f.write(contract_address)
        f.close()
    print(
        '\033[1m[ ] Contract address saved in file: \033[0m\033[92m%s \033[0m' %
        ('./out/' + filename_write_address))
...
#    sys.stdout.flush()
    ...
    receipt = MyGlobals.web3.eth.getTransactionReceipt(
...
def predict_contract_address(accountAddress):
    ...
    adr = Web3.sha3(rlp.encode(
        [normalize_address(accountAddress), nonce]), encoding='bytes')[-40:]
    return '0x' + adr
